...
async def get_user_by_username(session, username: str) -> UserLogin | None:
...
async def get_user_by_id(session, id: int) -> User | None:
    user = await session.get(User, id)
    return user


async def get_user_by_email(session, email: EmailStr) -> UserLogin | None:
...
async def create_user(session, user: UserCreate) -> UserRead:
